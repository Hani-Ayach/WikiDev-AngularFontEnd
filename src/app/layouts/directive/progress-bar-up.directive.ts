...
import { BehaviorSubject, combineLatest, switchMap, animationFrameScheduler, interval, map, takeWhile, endWith, distinctUntilChanged, takeUntil, take, tap } from 'rxjs';
/**
 * Quadratic Ease-Out Function: f(x) = x * (2 - x)
 */
 const easeOutQuad = (x: number): number => x * (2 - x);
@Directive({
  selector: '[progressBarUp]'
})
export class ProgressBarUpDirective {
